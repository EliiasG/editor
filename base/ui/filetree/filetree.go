...
// copied from fyne-x filtree, and modified
...
type fileTreeLabel struct {
	widget.Label
	rightClick func(*fyne.PointEvent)
	leftClick  func()
}

func (l *fileTreeLabel) Tapped(_ *fyne.PointEvent) {
	l.leftClick()
}

func (l *fileTreeLabel) TappedSecondary(e *fyne.PointEvent) {
	l.rightClick(e)
}

func newFileTreeLabel(text string) fyne.CanvasObject {
	lbl := &fileTreeLabel{}
...
type FileTree struct {
	...
	RightClicked func(widget.TreeNodeID, *fyne.PointEvent)
...
func NewFileTree(root fyne.URI) *FileTree {
	tree := &FileTree{
		Tree: widget.Tree{
			...
			CreateNode: func(branch bool) fyne.CanvasObject {
				...
				return container.NewBorder(nil, nil, icon, nil, newFileTreeLabel("Template Object"))
			},
		},
	...
	tree.UpdateNode = func(id widget.TreeNodeID, branch bool, node fyne.CanvasObject) {
		...
		lbl := c.Objects[0].(*fileTreeLabel)
		lbl.SetText(l)
		lbl.rightClick = func(e *fyne.PointEvent) {
			if tree.RightClicked != nil {
				tree.RightClicked(id, e)
			}
		}
		lbl.leftClick = func() {
			tree.Select(id)
